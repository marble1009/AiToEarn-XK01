...
(async () => {
  try {
    ...
    // Type `sudo docker compose restart aitoearn-nginx`
    console.log('🔄 Restarting aitoearn-nginx to refresh dynamic IP cache...');
    await terminalPage.keyboard.type('sudo docker compose restart aitoearn-nginx\n', { delay: 40 });
    await new Promise(r => setTimeout(r, 8000));

    const artifactDir = 'C:\\Users\\Admin\\.gemini\\antigravity\\brain\\7c16e4e3-439e-4dd6-b1e6-a7b47ace866d';
    const screenshotPath = path.join(artifactDir, 'scratch', 'restart_nginx.png');
    ...
    console.error('❌ Error during restart execution:', err);
  }
})();
