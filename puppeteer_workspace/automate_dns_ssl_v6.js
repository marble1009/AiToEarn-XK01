...
(async () => {
  ...
  try {
    ...
    async function deployDNSRecord(rec) {
      ...
      // VERY IMPORTANT: Wait 3 seconds for React to completely re-mount and re-render cell 5!
      console.log('   - Waiting 3 seconds for React input re-mount after dropdown selection...');
      await new Promise(r => setTimeout(r, 3000));

      // 6. Enter token value in activeTr's Value input using native property setter in a retry loop!
      console.log('   - Setting token value into Value field with re-mount protection...');
      let setSuccess = false;
      for (let attempt = 1; attempt <= 3; attempt++) {
        const valInput = await activeTr.$('input[name="Value"]') || await tds[5].$('input');
        if (valInput) {
          await page.evaluate((inputEl, textVal) => {
            const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
            nativeInputValueSetter.call(inputEl, textVal);
          ...
          const actualVal = await page.evaluate(el => el.value, valInput);
          if (actualVal === rec.value) {
            console.log(`     (Attempt ${attempt}) Set value verified successfully!`);
            setSuccess = true;
            break;
          } else {
            console.warn(`     (Attempt ${attempt}) Verification failed. Value is "${actualVal}". Retrying...`);
          }
        }
        await new Promise(r => setTimeout(r, 1000));
      }
      ...
      if (!setSuccess) {
        throw new Error('❌ Failed to set and verify Value input field!');
      }
