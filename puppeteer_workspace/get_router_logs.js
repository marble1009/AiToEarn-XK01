...
(async () => {
  console.log('🚀 Fetching router-specific logs...');
  try {
    ...
    // Fetch filtered logs
    const logsCmd = "sudo docker compose logs --tail=1000 aitoearn-ai | grep -i -E 'ClaudeCodeRouterService|Router' -C 2";
    ...
    const diagPath = path.join(artifactDir, 'cloud_router_logs.png');
    await orcaPage.screenshot({ path: diagPath });
    console.log(`📸 Router logs screenshot saved to: ${diagPath}`);

    await browser.disconnect();
    console.log('Done!');
  } catch (err) {
    console.error('Error during router logs diagnostics:', err);
  }
})();
