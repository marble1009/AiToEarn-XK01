...
(async () => {
  console.log('🚀 正在启动针对 OrcaTerm 卡死修复的直接 Docker 构建与测试脚本...');
  const artifactsDir = 'C:\\Users\\Admin\\.gemini\\antigravity\\brain\\7c16e4e3-439e-4dd6-b1e6-a7b47ace866d';

  let browser;
  try {
    console.log('🔌 尝试通过 9222 调试端口秒级连接常驻 Chrome...');
    browser = await puppeteer.connect({
      browserURL: 'http://127.0.0.1:9222',
      defaultViewport: null
    });
    console.log('🔌 成功秒级连入常驻 Chrome 实例 (9222)!');
  } catch (e) {
    console.log('ℹ️ 调试端口未响应。正在通过 Puppeteer 强力启动全新的有头 Chrome 调试窗口...');
    browser = await puppeteer.launch({
      ...
      args: [
        ...
        '--user-data-dir=C:\\Users\\Admin\\.gemini\\antigravity\\chrome-profile',
        '--no-first-run'
      ]
    });
    console.log('🎉 全新有头 Chrome 实例成功拉起并侦听 9222 调试端口！');
  }
  ...
  try {
    ...
    async function sendCommand(cmd, waitMs = 3000) {
      ...
      await orcaPage.keyboard.type(cmd + '\n', { delay: 60 });
      console.log(`   ⏳ 等待 ${waitMs / 1000} 秒以确保指令执行完成...`);
      await new Promise(r => setTimeout(r, waitMs));
    }
    
    // 1. 发送 Ctrl+C 强行切断当前卡死的输入状态
    console.log('🛑 正在向 OrcaTerm 发送 Ctrl + C 强行切断当前卡住的输入行...');
    ...
    // 1.5. 强力进入项目仓库目录（必须在该目录下执行 Docker 构建，否则会报 path not found！）
    console.log('📂 正在导航进入云服务器项目仓库目录...');
    await sendCommand('cd ~/aitoearn || cd aitoearn || cd Desktop/aitoearn', 6000);

    // 2. 直接执行 Docker 构建 (因为 git 已经 reset --hard 到了最新代码)
    console.log('🏗️ 代码已为最新。正在直接执行云服务器本地 Docker 镜像编译 (安全时间 240 秒)...');
    ...
    console.log('🔌 正在断开与 Chrome 的 Puppeteer 连接。常驻 Chrome 将绝对保持打开！');
