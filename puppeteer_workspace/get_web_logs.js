...
(async () => {
  try {
    ...
    // Type `sudo docker compose logs --tail=40 aitoearn-web`
    console.log('📋 Fetching web container logs...');
    await terminalPage.keyboard.type('sudo docker compose logs --tail=40 aitoearn-web\n', { delay: 40 });
    await new Promise(r => setTimeout(r, 8000));

    const artifactDir = 'C:\\Users\\Admin\\.gemini\\antigravity\\brain\\7c16e4e3-439e-4dd6-b1e6-a7b47ace866d';
    const screenshotPath = path.join(artifactDir, 'scratch', 'web_logs.png');
    await terminalPage.screenshot({ path: screenshotPath });
    console.log(`📸 Web logs screenshot saved to: ${screenshotPath}`);
