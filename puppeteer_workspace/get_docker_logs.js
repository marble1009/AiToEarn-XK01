...
(async () => {
  console.log('🚀 Connecting to OrcaTerm on port 9222 to run diagnostics...');
  try {
    ...
    // 1. Run docker compose ps to see current status
    console.log('⌨️ Typing: sudo docker compose ps');
    await orcaPage.keyboard.type('sudo docker compose ps', { delay: 50 });
    await orcaPage.keyboard.press('Enter');
    await new Promise(r => setTimeout(r, 3000));

    // 2. Fetch the last 50 lines of logs for aitoearn-ai
    console.log('⌨️ Typing: sudo docker compose logs --tail=50 aitoearn-ai');
    await orcaPage.keyboard.type('sudo docker compose logs --tail=50 aitoearn-ai', { delay: 50 });
    await orcaPage.keyboard.press('Enter');
    ...
    const diagPath = path.join(artifactDir, 'cloud_diag_logs.png');
    ...
    console.error('Error during cloud diagnostics:', err);
  }
})();
