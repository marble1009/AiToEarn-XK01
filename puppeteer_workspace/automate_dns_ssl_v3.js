...
// DNS Query Helper using authoritative DNSPod Nameservers directly (bypasses any resolver caches)
function checkAuthoritativeDns(host, expectedValue) {
  return new Promise((resolve) => {
    const resolver = new dns.Resolver();
    // Direct public IPs of experience.dnspod.net and peanut.dnspod.net
    resolver.setServers([
      '1.12.0.4',
      '1.14.119.35',
      '1.12.0.1',
    ...
    console.log(`🔍 Querying DNSPod Authoritative Nameservers directly for ${fullDomain}...`);
    
    resolver.resolveTxt(fullDomain, (err, addresses) => {
      if (err) {
        console.log(`   [Authoritative DNS Error]: ${err.code}`);
        resolve(false);
      } else {
        console.log(`   [Authoritative DNS Answers]:`, JSON.stringify(addresses));
        for (const record of addresses) {
          const val = record.join('').trim();
...
async function waitForDnsPropagation(host, expectedValue) {
  ...
  for (let attempt = 1; attempt <= 30; attempt++) {
    const isPropagated = await checkAuthoritativeDns(host, expectedValue);
    if (isPropagated) {
      ...
      while ((match = regex.exec(outputBuffer)) !== null) {
        ...
        if (!processedChallenges.has(uniqueKey)) {
          ...
          try {
            ...
            // 4. Robust Authoritative DNS Propagation Checking
