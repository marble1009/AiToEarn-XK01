...
// DNS Query Helper using Google DoH API
function checkGlobalDns(host, expectedValue) {
  return new Promise((resolve) => {
    const fullDomain = `${host}.aiautoedit.art`;
    const url = `https://dns.google/resolve?name=${encodeURIComponent(fullDomain)}&type=TXT`;
    
    console.log(`🔍 Querying Google DoH for TXT record of ${fullDomain}...`);
    https.get(url, (res) => {
      let data = '';
        ...
        try {
          ...
          if (json && json.Answer) {
            for (const ans of json.Answer) {
              if (ans.type === 16) { // TXT record type
                const val = ans.data.replace(/"/g, '').trim();
                console.log(`   [Google DoH Answer]: ${val}`);
          ...
          console.error(`   [DoH Error parsing JSON]:`, e.message);
          resolve(false);
        }
      });
    }).on('error', (err) => {
      console.error(`   [DoH Network Error]:`, err.message);
      resolve(false);
    });
  });
}

// Wait until DNS record is propagated globally
async function waitForDnsPropagation(host, expectedValue) {
  console.log(`⏳ Starting propagation check for ${host}.aiautoedit.art -> ${expectedValue}`);
  // Wait up to 10 minutes (30 attempts * 20s)
  for (let attempt = 1; attempt <= 30; attempt++) {
    const isPropagated = await checkGlobalDns(host, expectedValue);
    if (isPropagated) {
      console.log(`🎉 DNS successfully propagated globally!`);
      // Wait another 5 seconds for absolute certainty
      await new Promise(r => setTimeout(r, 5000));
      return true;
    }
    console.log(`   (Attempt ${attempt}/30) Not propagated yet. Retrying in 20 seconds...`);
    await new Promise(r => setTimeout(r, 20000));
  }
  console.warn(`⚠️ Propagation check timed out after 10 minutes. Continuing anyway...`);
  return false;
}
...
(async () => {
  ...
  try {
      ...
      while ((match = regex.exec(outputBuffer)) !== null) {
        ...
        if (!processedChallenges.has(uniqueKey)) {
          ...
          try {
            ...
            // 4. Robust Google DoH Propagation Checking
    ...
    async function deployDNSRecord(rec) {
      ...
      // 3. Fill in host record name in tds[2] using extremely robust direct React property input setting
      ...
      // 6. Enter token value in tds[5] using robust direct React property input setting
