...
export function PhoneLoginForm({ onLoginSuccess, redirectUrl, inviteCode: _inviteCodeProp }: PhoneLoginFormProps = {}) {
  ...
  return (
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
      <div>
        <div className="flex">
          <span className="inline-flex h-12 items-center rounded-l-xl border border-r-0 border-border bg-black px-3 text-sm text-foreground font-bold">
            +86
          </span>
          <Input
            type="tel"
            ...
            {...form.register('phone')}
            className="h-12 rounded-l-none rounded-r-xl border border-border bg-black/60 px-4 text-base text-foreground placeholder:text-muted-foreground/50 focus:border-primary transition-all outline-none"
          />
        </div>
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          className="h-12 shrink-0 cursor-pointer rounded-xl px-4 border border-border bg-black text-foreground hover:bg-accent hover:text-accent-foreground transition-all"
        >
          {sendingCode ? (
